...
const DATA_INVENTORY = {
  raw: [
    { name: 'conagua', size: '--', desc: 'Títulos de concesión REPDA y registros hídricos de almacenamiento de presas.', tag: 'raw' },
    { name: 'datos-abiertos', size: '--', desc: 'Datos abiertos gubernamentales de agricultura y recursos de Sonora.', tag: 'raw' },
    { name: 'datos-sequia', size: '820 KB', desc: 'Reportes de anomalías de lluvia y Monitor de Sequía mensual de CONAGUA.', tag: 'raw' },
    { name: 'sader', size: '--', desc: 'Registros de DDRs y oficinas delegacionales del estado de Sonora.', tag: 'raw' },
    { name: 'siacon', size: '45 MB', desc: 'Base de datos del Sistema de Información Agroalimentaria y Pesquera de SADER.', tag: 'raw' },
    { name: 'siap-produccion-agricola', size: '120 MB', desc: 'Estadísticas históricas completas de producción municipal de la SIAP.', tag: 'raw' }
  ],
  processed: [
    { name: 'SonoraLatLongAlt.csv', size: '3.6 KB', desc: 'Listado geográfico de coordenadas y altitudes promedio para municipios de Sonora.', tag: 'processed' },
    { name: 'monitor_sequia_sonora.csv', size: '837 KB', desc: 'Historial limpio y depurado del monitor de sequía a nivel municipal.', tag: 'processed' },
    { name: 'sequia_indices_sonora.csv', size: '9.2 KB', desc: 'Índices municipales calculados de severidad y frecuencia de sequías.', tag: 'processed' },
    { name: 'analisis_municipal_sonora_2010_2024.csv', size: '11 MB', desc: 'Dataset consolidado de producción y clima a nivel municipal listo para ML.', tag: 'processed' },
    { name: 'nasa_power/', size: 'Variable', desc: 'Archivos CSV históricos diarios de clima descargados por municipio y año.', tag: 'processed' },
    { name: 'siap_produccion/sonora/', size: '2.5 MB', desc: 'Archivos CSV anuales de producción agrícola de la SIAP limpios para Sonora.', tag: 'processed' }
  ]
};
  ...
  children: [
    {
      ...
      type: "dir",
      children: [
        { name: "data_models.py", type: "file", desc: "Clases base Crop y Region modeladas con Pydantic para tipado estricto." }
      ]
    },
    {
      name: "datasets",
      type: "dir",
      children: [
        { name: "codes.py", type: "file", desc: "Manejo y decodificación de códigos municipales y DDRs." },
        { name: "data.py", type: "file", desc: "API de carga y filtrado de datos climáticos y agrícolas." },
        { name: "paths.py", type: "file", desc: "Constantes y rutas absolutas a archivos crudos y procesados." },
        { name: "repository.py", type: "file", desc: "Módulo I/O para aislamiento físico del almacenamiento de datos." }
      ]
    },
    {
      name: "analysis",
      type: "dir",
      children: [
        { name: "pipeline.py", type: "file", desc: "Flujo de cálculo completo de ETo, huella hídrica y reconversión." }
      ]
    },
    {
      name: "utils",
      type: "dir",
      children: [
        { name: "date.py", type: "file", desc: "Cálculo de días del año y periodos de ciclos agrícolas." },
        { name: "eto.py", type: "file", desc: "Fórmulas matemáticas FAO-56 Penman-Monteith y Hargreaves." },
        { name: "nasa_power.py", type: "file", desc: "Cliente para descargas HTTP automatizadas desde NASA POWER API." },
        { name: "performance.py", type: "file", desc: "Perfilador y decoradores de rendimiento para optimizar cálculo." },
        { name: "validation.py", type: "file", desc: "Funciones de control de esquemas de datos." }
      ]
    }
  ]
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch('data_content.json')
    ...
    .catch(err => {
      ...
      document.getElementById("loader").innerHTML = `<div style="text-align:center;color:red;padding:2rem;">
...
function renderAllDynamicContent() {
  ...
  if (estContentDiv && AppState.data.reporte_est) {
    estContentDiv.innerHTML = renderReportSectionsHTML(AppState.data.reporte_est);
  }
  ...
  sections.forEach(sec => {
    ...
    sec.blocks.forEach(block => {
      if (block.type === "paragraph") {
        ...
        html += `<ul class="list-premium"><li>${block.content}</li></ul>`;
...
function renderInventory() {
  ...
  if (rawContainer) {
    rawContainer.innerHTML = DATA_INVENTORY.raw.map(file => `
      <div class="card">
  ...
  if (processedContainer) {
    processedContainer.innerHTML = DATA_INVENTORY.processed.map(file => `
      <div class="card">
...
function setupCalculatorListeners() {
  const inputs = ["calc-tmax", "calc-tmin", "calc-rh", "calc-u2", "calc-rs", "calc-precip", "calc-rend"];
  inputs.forEach(id => {
...
function calculateFAO56Simulation() {
  ...
  let kc_ini = 0.3, kc_mid = 1.15, kc_end = 0.25;
  ...
  document.getElementById("res-etc").textContent = `${etc.toFixed(2)} mm/día (Kc avg: ${kc_avg.toFixed(2)})`;
  document.getElementById("res-uac-green").textContent = `${Math.round(uac_verde).toLocaleString()} m³/Ha`;
  document.getElementById("res-uac-blue").textContent = `${Math.round(uac_azul).toLocaleString()} m³/Ha`;
  document.getElementById("res-hh-green").textContent = `${Math.round(hh_verde).toLocaleString()} m³/Ton`;
  document.getElementById("res-hh-blue").textContent = `${Math.round(hh_azul).toLocaleString()} m³/Ton`;
  document.getElementById("res-hh-total").textContent = `${Math.round(hh_total).toLocaleString()} m³/Ton`;
}
...
function renderCharts() {
  ...
  AppState.charts.efficiency = new Chart(ctxEff, {
    type: 'bar',
    data: {
      labels: ['Col (repollo)', 'Coliflor', 'Hortalizas', 'Caña', 'Zanahoria', 'Trigo (grano)', 'Alfalfa (forrajera)', 'Frijol', 'Nuez'],
      datasets: [{
        label: 'Eficiencia Hídrica (Ton/m³)',
        data: [0.0375, 0.0213, 0.0166, 0.0148, 0.0127, 0.0021, 0.0012, 0.00017, 0.0001],
        backgroundColor: [
        ...
        borderColor: [
          '#a3e635', '#a3e635', '#a3e635', '#a3e635', '#a3e635',
          '#38bdf8', '#38bdf8', '#fb923c', '#fb923c'
        ],
        borderWidth: 1.5
      }]
    },
  ...
  AppState.charts.economic = new Chart(ctxEco, {
    type: 'line',
    data: {
      labels: ['Agua Prieta', 'Caborca', 'Cajeme', 'Guaymas', 'Hermosillo', 'Magdalena', 'Navojoa'],
      datasets: [{
